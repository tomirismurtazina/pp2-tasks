...
def fill(surface, x, y, color, w, h):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if typing:
                if event.key == pygame.K_RETURN:
                    msg = font.render(text_msg, True, color)
                    screen.blit(msg, text_pos)
                    typing = False
                    text_msg = ""
                    ...
                    if event.unicode and event.unicode.isprintable():
                        text_msg += event.unicode
                        temp_msg = font.render(text_msg, True, color)
                        base_layer.blit(temp_msg, text_pos)
                continue
            if event.key == pygame.K_s and event.mod and pygame.KMOD_LCTRL:
                date = datetime.strftime(datetime.now(), "%Y-%m-%d")
                pygame.image.save(screen, f"paint{date}.png")
                print("File saved")
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if tool == "text":
                ...
                text_pos = event.pos
            if tool == "fill":
                fill(screen, currX, currY, color, WIDTH, HEIGHT)
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1:
                if tool == "rect":
                    ...
                    pygame.draw.circle(screen, color, (prevX, prevY), r, THICKNESS)
                    ...
                    h = s * math.sqrt(3) / 2
                    m = (currX + (s/2), currY)
                    pygame.draw.polygon(screen, color, ((prevX, currY), (currX, currY), (currX + (s/2), currY + h)), THICKNESS)

                elif tool == "right triangle":
                    pygame.draw.polygon(screen, color, ((prevX, prevY), (currX, currY), (prevX, currY)), THICKNESS)
